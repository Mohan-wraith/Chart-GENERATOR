...
🧠 How It Works
User Input: The user searches for a show (e.g., "Breaking Bad").
...
Data Processing: Ratings are normalized, and missing data points are handled gracefully.

Rendering: The render_page() function creates a blank canvas and draws the heatmap pixel-by-pixel, calculating layout dynamically based on the number of seasons and episodes.

Output: A high-quality PNG is displayed and offered for download.
...
Since your `app.py` has a specific import list, make sure your **`requirements.txt`** looks like this so people can actually run it:
